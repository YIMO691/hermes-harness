...
| Harness Checker | `tools/check_workline_task.py` — 10/10 cases |
| Skill 结构 | 单文件 Workline Core |
| Git 工作流 | Workline Git Workflow v1 — develop 分支已执行 |
| LangGraph / LangChain | 未引入 |
| 回归套件 | 10 个用例（9 验证，1 待补） |

**证据**：v1.7-stable 三项验证通过。v1.8-stable checker + fixtures。v1.9-stable CI + regression 10/10 + Git workflow 执行。
...
│   └── V1.9_CI_REGRESSION_EVIDENCE_PLAN.md        ← v1.9 计划
├── tools/
...
**当前阶段**：v1.9 稳定使用期。不出现真实断裂点，不启动 v1.10。

v1.10 候选方向：
- 更多真实 Unity 任务验证
- Harness Health Report / metrics 跨任务聚合
- token observability 设计或最小接入

**不能触发 v1.10 的情况**：觉得架构不够漂亮、想接 LangGraph、想激活全部 Gate、想做 Dashboard。
